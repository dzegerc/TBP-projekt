...
app.post('/api/prijava', async (req, res) => {
  ...
  try {
    ...
    // Provjeri lozinku (sada bez hashiranja)
    ...
    // Generiraj JWT token ako je prijava uspješna
...
const authMiddleware = (req, res, next) => {
  try {
    console.log('Authorization Header:', req.headers.authorization); // Dodajte ovo
    ...
    console.log('Token:', token); // Dodajte ovo
    const decodedToken = jwt.verify(token, 'tajna_lozinka');
    console.log('Decoded Token:', decodedToken); // Dodajte ovo

    req.userId = decodedToken.korisnikId;
    next();
  } catch (error) {
    console.error('Greška u authMiddleware:', error); // Ovdje možete vidjeti detalje greške
    res.status(401).json({ poruka: 'Niste prijavljeni ili vam je sesija istekla' });
  }
};


// Koristite authMiddleware za rute koje zahtijevaju autentikaciju
app.use('/api/tasks', authMiddleware);
app.get('/api/korisnici', authMiddleware);
app.get('/api/korisnici/:id', authMiddleware);


//dohvaćanje svih taskova
...
app.get('/api/korisnici/:id', async (req, res) => {
  ...
  const token = authHeader.split(' ')[1]; // Pretpostavka da je format "Bearer [token]"
  if (!token) {
...
app.get('/api/moderator/korisnici', authMiddleware, async (req, res) => {
  try {
    const { userId } = req;  // ID trenutnog korisnika (moderatora)
    ...
    // Upit za dohvat korisnika koji su dodijeljeni trenutnom moderatoru
    const query = `
...
//ažuriranje taskova od strane moderatora i administratora
...
// GET endpoint za dohvaćanje uloga
